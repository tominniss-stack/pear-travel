...
export default function CalendarExportModal({ 
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-zinc-900 rounded-3xl shadow-xl border border-zinc-200 dark:border-zinc-800 p-8 text-center animate-in fade-in zoom-in-95 duration-200 text-zinc-900 dark:text-white">
        
        <span className="text-5xl mb-4 block drop-shadow-md">📅</span>
        
        {!trip.startDate ? (
          <>
            <h3 className="text-xl font-black mb-2 text-zinc-900 dark:text-white">Lock Your Dates</h3>
          ...
          <>
            <h3 className="text-xl font-black mb-2 text-zinc-900 dark:text-white">Export Calendar</h3>
            <p className="text-sm font-medium text-zinc-500 mb-6">
        ...
        <form onSubmit={handleExport} className="flex flex-col gap-4">
          ...
          {!trip.startDate && (
            <div className="text-left">
              <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1.5 pl-1">Trip Start Date</label>
              <input 
                type="date" 
                required
                value={startDateStr}
                onChange={(e) => setStartDateStr(e.target.value)}
                className="w-full px-4 py-3.5 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl font-medium text-zinc-900 dark:text-white transition-all cursor-pointer"
              />
            </div>
          )}
          ...
          <div className="flex gap-3 mt-2">
            <button 
              type="button" 
              onClick={onClose} 
              className="flex-1 py-3.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 font-medium rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
            >
              Cancel
            </button>
            <button 
              type="submit" 
              disabled={isLoading}
              className="flex-[2] py-3.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-medium rounded-full hover:bg-zinc-800 dark:hover:bg-zinc-200 active:scale-[0.98] transition-all disabled:opacity-50"
            >
