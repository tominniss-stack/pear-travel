...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
      ...
      <div>
        <p className="text-sm font-semibold text-slate-600">No places added yet</p>
        <p className="mt-0.5 text-xs text-slate-400 leading-relaxed">Tap the heart on any card to add it to your trip.</p>
      </div>
    </div>
  );
}
...
function StagedPOIRow({ poi, onRemove }: StagedPOIRowProps) {
  return (
    <li className="flex items-start gap-3 rounded-xl bg-slate-50 p-3 transition-colors hover:bg-slate-100">
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-white shadow-sm text-base">
        {getCategoryEmoji(poi.category)}
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-slate-700 leading-snug">{poi.name}</p>
      ...
      <button type="button" onClick={() => onRemove(poi.placeId)} aria-label={`Remove ${poi.name} from trip`} className="flex-shrink-0 rounded-lg p-1 text-slate-300 transition-colors hover:bg-red-50 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-red-400">
...
export default function StagingArea({ tripId }: StagingAreaProps) {
  ...
  const innerContent = (
    <>
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <h2 className="text-base font-bold text-slate-800">Your Trip</h2>
          <p className="text-xs text-slate-400 mt-0.5">{count === 0 ? 'No places selected' : `${count} place${count !== 1 ? 's' : ''} selected`}</p>
        </div>
        {count > 0 && (
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-brand-600 text-xs font-bold text-white shadow-sm">{count}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3 border-t border-slate-100 pt-4">
        {count > 0 && (
          <div className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2.5">
            <span className="text-xs font-medium text-slate-500">Est. entry costs</span>
            <span className="text-sm font-bold text-slate-700">{totalCost === 0 ? 'All free!' : `£${totalCost.toLocaleString('en-GB')}`}</span>
          </div>
        )}
        <button 
          type="button" 
          ...
          className="relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl px-4 py-3.5 text-sm font-bold tracking-wide transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 bg-brand-600 text-white shadow-md hover:bg-brand-700 active:scale-[0.98]"
        >
          {isNavigating ? (
        ...
        {count === 0 && <p className="text-center text-xs text-slate-400">Add at least one place to continue.</p>}
      </div>
    </>
  );
  ...
  return (
    <>
      <aside aria-label="Trip staging area" className="hidden lg:flex flex-col gap-4 sticky top-6 h-[calc(100vh-3rem)] max-h-[800px] w-80 flex-shrink-0 overflow-hidden rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        {innerContent}
      </aside>

      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40">
        {mobileExpanded && <div className="fixed inset-0 bg-black/20 backdrop-blur-sm" onClick={() => setMobileExpanded(false)} aria-hidden="true" />}
        <div className={`relative flex flex-col rounded-t-2xl border-t border-slate-200 bg-white shadow-2xl transition-all duration-300 ease-out ${mobileExpanded ? 'max-h-[80vh]' : 'max-h-24'}`}>
          <button type="button" onClick={() => setMobileExpanded((v) => !v)} aria-expanded={mobileExpanded} aria-label={mobileExpanded ? 'Collapse trip panel' : 'Expand trip panel'} className="flex w-full flex-col items-center gap-2 px-5 pt-3 pb-3 focus:outline-none">
            ...
            <div className="flex w-full items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-600 text-xs font-bold text-white">{count}</span>
                <span className="text-sm font-semibold text-slate-700">{count === 0 ? 'No places added yet' : `${count} place${count !== 1 ? 's' : ''} selected`}</span>
              </div>
              <div className="flex items-center gap-3">
                {count > 0 && <span className="text-sm font-bold text-slate-600">£{totalCost.toLocaleString('en-GB')}</span>}
                <svg className={`h-4 w-4 text-slate-400 transition-transform duration-300 ${mobileExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" aria-hidden="true">
