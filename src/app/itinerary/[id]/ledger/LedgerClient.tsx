...
export default function LedgerClient({ trip, initialItinerary }: LedgerClientProps) {
  ...
  // ── Form State for New Misc Expense ──
  ...
  // ── Inline Editing State for Timeline Actuals ──
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-[340px] flex-shrink-0 lg:sticky lg:top-28 flex flex-col gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-sm min-w-0">
              ...
              <div className="h-3 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                <div 
                  className={`h-full transition-all duration-1000 ${totalActiveSpend > trip.budgetGBP ? 'bg-red-500' : 'bg-brand-500'}`}
                  style={{ width: `${Math.min((totalActiveSpend / trip.budgetGBP) * 100, 100)}%` }}
                />
              </div>
            </div>
              ...
              <div className="flex h-3 w-full rounded-full overflow-hidden mb-5 bg-slate-100 dark:bg-slate-800 shadow-inner gap-[1px]">
                {Object.entries(categoryBreakdown).map(([cat, amount]) => {
                  ...
                  return (
                    <div 
                      key={cat} 
                      className={`h-full ${CATEGORY_COLORS[cat as ExpenseCategory]}`} 
                      style={{ width: `${percentage}%` }}
                      title={`${cat}: ${formatCost(amount)}`}
                    />
                  );
                })}
              </div>
            ...
            <div className="bg-slate-900 dark:bg-slate-950 rounded-3xl p-6 md:p-8 shadow-xl text-white border border-slate-800 relative overflow-hidden min-w-0">
              ...
              <div className="relative z-10 min-w-0">
                ...
                <p className="text-xs text-slate-400 font-medium leading-relaxed mb-4 border-b border-slate-800 pb-4">
                  Includes {formatCost(prePaidSpend)} in pre-paid / advance expenses.
                </p>
                <div className="flex items-start gap-2">
                  <span className="text-lg">ℹ️</span>
                  <p className="text-[10px] text-slate-500 leading-tight">
                    * Conversions estimated at <strong className="text-slate-400">£1 = {localSymbol}{exchangeRate.toFixed(2)}</strong> (rate at start of trip).
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 w-full flex flex-col gap-6 min-w-0">
            ...
            <div className="flex gap-2 bg-slate-200/50 dark:bg-slate-800/50 p-1.5 rounded-2xl w-max border border-slate-200/50 dark:border-slate-700/50">
              <button onClick={() => setActiveTab('timeline')} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'timeline' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                Itinerary Timeline
              </button>
              <button onClick={() => setActiveTab('misc')} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'misc' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                Pre-paid & Misc {(activeItinerary.miscExpenses?.length || 0) > 0 && <span className="bg-slate-200 dark:bg-slate-800 px-2 py-0.5 rounded-md text-[10px]">{activeItinerary.miscExpenses?.length}</span>}
              </button>
            </div>
                        ...
                        {editingEntryId !== item.id && (
                          <button 
                            onClick={() => {
                              ...
                              setEditCurrency(activeLocalCurrency);
                              setEditAmount(activeLocalCurrency === 'LOCAL' && !isDomesticTrip ? (item.appliedCost * exchangeRate).toFixed(2) : item.appliedCost.toFixed(2));
                              setEditDocId(item.linkedDocumentId || '');
                            }}
                          ...
                          >
                            <span className="opacity-60 font-medium mr-1">{item.actualCostGBP !== undefined ? 'Actual:' : 'Est:'}</span> 
                            {formatCost(item.appliedCost)}
                          </button>
                        )}
                      </div>

                      {/* Clean Expandable Editor */}
                      {editingEntryId === item.id && (
                          ...
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-stretch bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-brand-500 shadow-sm">
                                {!isDomesticTrip && (
                                  <button 
                                    onClick={() => setEditCurrency(prev => prev === 'LOCAL' ? 'GBP' : 'LOCAL')}
                                    className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-xs font-black text-slate-700 dark:text-slate-300 hover:bg-slate-200 transition-colors border-r border-slate-300 dark:border-slate-600 cursor-pointer flex-shrink-0"
                                  >
                                ...
                                <input 
                                  ...
                                  className="flex-1 min-w-0 px-4 py-2.5 bg-transparent text-sm font-bold text-slate-900 dark:text-white outline-none tabular-nums"
                                  placeholder="0.00"
                                />
                              </div>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <select 
                                ...
                                className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-brand-500 text-slate-900 dark:text-white shadow-sm"
                              >
            ...
            {activeTab === 'misc' && (
              <div className="flex flex-col gap-6">
                {!isAdding ? (
                  <button onClick={() => setIsAdding(true)} className="w-full py-6 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-3xl text-slate-500 hover:border-brand-500 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/10 transition-all font-bold flex items-center justify-center gap-2 shadow-sm cursor-pointer">
                    <span className="text-xl leading-none mb-0.5">+</span> Log Manual / Pre-paid Expense
                  </button>
                ) : (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                      ...
                      <div>
                        ...
                        <div className="flex items-stretch bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-brand-500">
                          {!isDomesticTrip && (
                            <button type="button" onClick={() => setInputCurrency(prev => prev === 'LOCAL' ? 'GBP' : 'LOCAL')} className="px-5 py-3 bg-slate-200 dark:bg-slate-700 text-xs font-black text-slate-700 dark:text-slate-300 hover:bg-slate-300 transition-colors border-r border-slate-300 dark:border-slate-600 cursor-pointer flex-shrink-0">
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                  {(activeItinerary.miscExpenses || []).length === 0 ? (
                    <div className="p-10 text-center flex flex-col items-center">
                      ...
                      <p className="text-slate-500 font-medium">No manual expenses logged yet.</p>
                    </div>
                  ) : (
                    (activeItinerary.miscExpenses || []).map((exp: MiscExpense) => (
                      <div key={exp.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-5 w-full sm:w-auto flex-shrink-0">
                          ...
                          <button 
                            onClick={() => {
                              removeMiscExpense(exp.id);
                              syncLedger(); // Permanently delete from DB
                            }} 
                            className="w-10 h-10 rounded-full bg-red-50 dark:bg-red-900/20 text-red-500 flex items-center justify-center hover:bg-red-100 transition-colors flex-shrink-0 shadow-sm cursor-pointer"
                          >
                            🗑
                          </button>
                        </div>
