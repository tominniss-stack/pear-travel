...
// Manual POI search endpoint — allows users to search for specific places
// on the discovery page and add them to their trip manually.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, destination } = body as { query: string; destination: string };
    ...
    const places = await fetchPlacesForQuery(query.trim(), destination.trim(), apiKey);

    if (places.length === 0) {
      return NextResponse.json({ poi: null }, { status: 200 });
    }

    // Return the top result
    const topPlace = places[0];
    const poi = mapGooglePlaceToPOI(topPlace, destination.trim());
